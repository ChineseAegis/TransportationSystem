...
public:
    MaxIndexHeap(Object arr[], int n) : capacity(n), size(n), data(n, n), indexes(n, n), reverse(n, n) {
        /* data = new E[n];
         indexes = new int[n];
         reverse = new int[n];*/
    ...
    MaxIndexHeap(int capacity = defaultSize) :capacity(capacity), size(0), data(capacity), indexes(capacity, capacity), reverse(capacity, capacity) {
        /*data = new E[capacity ];
